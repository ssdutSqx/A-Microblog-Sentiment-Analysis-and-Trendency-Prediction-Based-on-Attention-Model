...
word_list = []
with open('f:/SentimentAnalysisData/sgns.weibo.bigram-char','rb') as f:
    for lines in f:
...
neg_cnt = 0
with open("f:/SentimentAnalysisData/negatif.txt", 'rb') as f:
    for seg in f:
        ...
        name = "f:/SentimentAnalysisData/" + "neg_vec_" + str(neg_cnt)
        np.save(name,seg_vec_array)
        neg_cnt += 1
print("neg_cnt:",neg_cnt)

pos_cnt = 0
with open("f:/SentimentAnalysisData/positif.txt", 'rb') as f:
    for seg in f:
        seg = seg.decode('utf-8')
        seg_list = jieba.lcut(seg)
        seg_vec_list = []
        ...
        for word in seg_list:
            ...
            if index != -1:
                ...
                seg_vec_list.append(vec)
        seg_vec_array = np.array(seg_vec_list)
        name = "f:/SentimentAnalysisData/" + "pos_vec_" + str(pos_cnt)
        np.save(name, seg_vec_array)
        pos_cnt += 1
