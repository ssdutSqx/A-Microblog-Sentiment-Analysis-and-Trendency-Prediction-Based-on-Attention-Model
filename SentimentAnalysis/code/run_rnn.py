...
with tf.Session() as sess:
    ...
    for n in range(config.epoch):
        ...
        for train_index in batch_index_list:
            ...
            batch_loss_before = sess.run(rnn.loss, feed_dict=feed_batch)
            sess.run(rnn.AdamOptimize, feed_dict=feed_batch)
            batch_loss_after = sess.run(rnn.loss, feed_dict=feed_batch)
            del feed_batch

            train_loss = sess.run(rnn.loss, feed_dict=feed_train)
            train_loss_list.append(train_loss)
            test_loss = sess.run(rnn.loss, feed_dict=feed_test)
            test_loss_list.append(test_loss)

            train_accuracy = sess.run(rnn.accuracy, feed_dict=feed_train)
            train_accuracy_list.append(train_accuracy)
            test_accuracy = sess.run(rnn.accuracy, feed_dict=feed_test)
...
#accuracy.to_csv("LSTMModel_AdamOnly_mini-batch=%s_hidden-size=%s_num-layers=%s_dropout-keep=%s.csv"
...
plt.plot(range(0, i), train_accuracy_list)
plt.plot(range(0, i), test_accuracy_list)
plt.xlabel("round")
plt.ylabel('accuracy')
plt.title('LSTM mini-batch=%s' % config.mini_batch)
plt.show()